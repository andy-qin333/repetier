...
def index():
	...
	locales = dict((l.language+("_"+l.territory if l.territory is not None else ""), dict(language=l.language+("_"+l.territory if l.territory is not None else ""), territory=l.territory, display=l.display_name, english=l.english_name)) for l in LOCALES)
	...
	render_kwargs = dict(
		...
		heatedChamber=settings().get(["printerParameters", "heatedChamber"])
	)
