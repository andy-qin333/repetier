...
from octoprint.util import to_unicode, to_str
#~~ GCODE file handling
...
def readGcodeFiles():
	...
	FileDestinations.LOCAL = to_unicode(FileDestinations.LOCAL)
	files = _getFileList(FileDestinations.LOCAL, filter=filter)
	# zyd for usb storage -s
	files.extend(_getFileList(FileDestinations.USB, filter=filter))
...
def readGcodeFilesForOrigin(origin):
	...
	if origin not in [FileDestinations.LOCAL, FileDestinations.SDCARD, FileDestinations.USB]:
	# zyd for usb storage -e
			...
			file.update({
				"refs": {
					...
					#"download": url_for("index", _external=True) + "downloads/files/" + FileDestinations.LOCAL + "/" + file["name"]
			                ...
			                "download": url_for("index", _external=True) + "downloads/files/" + origin + "/" + to_unicode(file["name"])
			                # zyd for usb storage -e
				}
			})
	return files
...
def uploadGcodeFile(target):
	...
	filename = to_str(filename, encoding='utf-8')
	...
	files.update({
		FileDestinations.LOCAL: {
			...
			"refs": {
				"resource": location,
				"download": url_for("index", _external=True) + "downloads/files/" + FileDestinations.LOCAL + "/" + to_str(filename, encoding='utf-8')
			}
		}
	})
...
def readGcodeFile(target, filename):
	# zyd for usb storage -s
	#if not target in [FileDestinations.LOCAL, FileDestinations.SDCARD]:
	if not target in [FileDestinations.LOCAL, FileDestinations.SDCARD, FileDestinations.USB]:
	# zyd for usb storage -e
		return make_response("Unknown target: %s" % target, 404)
...
def gcodeFileCommand(filename, target):
	...
	if verifyResult and target == FileDestinations.USB:
		filename = fileManager.startThreadToCopyFile(filename, timeout = 3*60)
