...
class Server(object):
	def __init__(self, configfile=None, basedir=None, host="0.0.0.0", port=5000, debug=False, allowRoot=False, logConf=None, octoprint_daemon=None, sls=True):
	...
	def run(self):
		...
		storage_managers[octoprint.filemanager.FileDestinations.USB] = octoprint.filemanager.storage.LocalFileStorage(s.getUsbFolder("udisk"))
		...
		try:
			global localServer
			if self._startLocalServer in (True, "true"):
				try:
					from octoprint.localServer import localServerManager
				except:
					self._logger.error("There are some error, when import localServerManager")
				localServer = localServerManager()
		except:
			self._logger.error("There are some unexpected error, when start localServer")
	...
	def _setup_app(self):
		...
		def before_request():
			...
			if g.locale is not None and g.locale.territory is not None:
				g.locale.language = g.locale.language+"_"+g.locale.territory
