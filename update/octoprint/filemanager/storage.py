...
from octoprint.util import is_hidden_path, to_unicode, to_str # add by evan, for support chinese
class StorageInterface(object):
	...
	# zyd add for server file -s
	def add_server_file(self, filedata, model_id, model_version, model_name):
		raise NotImplementedError()
	
	def find_server_file(self, model_id, model_version):
		raise NotImplementedError()
	# zyd add for server file -e
...
class LocalFileStorage(StorageInterface):
	...
	def _analysis_backlog_generator(self, path=None):
		...
		for entry in os.listdir(path):
			...
			entry= to_str(entry, encoding='utf-8')
	...
	def file_exists(self, path):
		...
		path=to_str(path, encoding="utf-8")
		name=to_str(name, encoding="utf-8")
		file_path = os.path.join(path, name)
		return os.path.exists(to_str(file_path, encoding="utf-8")) and os.path.isfile(file_path)
	...
	def add_server_file(self, filedata, model_id, model_version, model_name):
		path = self.basefolder
		model_name = self.sanitize_name(model_name)
		metadata = self._get_metadata(path)
		if not metadata:
			metadata = dict()
		...
		# extract zip, get gcode file
		name = None
		if filedata:
			tmpzipfile = os.path.join(path, "temp.zip")
			with open(tmpzipfile, 'wb') as fzip:
				fzip.write(filedata)
			zfile = zipfile.ZipFile(tmpzipfile)
			...
			if files.count > 0:
				for f in zfile.namelist():
					...
					if extension in [".gcode",".3dt"]:
						...
						zfile.close()
						count = 0
						while os.path.isfile(os.path.join(path, model_name)):
							rootname, extension = os.path.splitext(model_name)
							model_name = rootname+"_"+str(count)+extension
							count += 1
						os.rename(os.path.join(path, name), os.path.join(path, model_name))
						break
			os.remove(tmpzipfile)
		if not name:
			raise RuntimeError("{name} does not exist in zip file")
		...
		metadata[model_name] = file_metadata
		self._save_metadata(path, metadata)

		# process any links that were also provided for adding to the file
		links = []

		self._add_links(model_name, path, links)
	...
	def find_server_file(self, model_id, model_version):
		...
		for name in metadata.keys():
			if "model_id" in metadata[name] and "model_version" in metadata[name]:
				if metadata[name]["model_id"] == model_id and metadata[name]["model_version"] == model_version:
	...
	def _save_metadata(self, path, metadata):
		path=to_str(path, encoding='utf-8')
		metadata_path = os.path.join(path, ".metadata.yaml")
		metadata_path=to_str(metadata_path, encoding='utf-8')
		with self._metadata_lock:
			try:
				import yaml
				import shutil
				...
				try:
					metadata=to_str(metadata, encoding='utf-8')
