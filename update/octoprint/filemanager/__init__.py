...
# zyd for usb storage -s
import threading
from time import sleep
# zyd for usb storage -e
...
def full_extension_tree():
	result = dict(
		...
		machinecode=dict(
			gcode=ContentTypeMapping(["gcode", "gco", "g", "3dt"], "text/plain")
		)
	)
...
class FileManager(object):
	...
	def add_server_file(self, destination, filedata, model_id, model_version, model_name):
		return self._storage(destination).add_server_file(filedata, model_id, model_version, model_name)
	
	def find_server_file(self, destination, model_id, model_version):
		return self._storage(destination).find_server_file(model_id, model_version)
	...
	# zyd for usb storage -s
	def startThreadToCopyFile(self, filename, timeout=0):
		#self._fileManager.path_in_storage(FileDestinations.USB, filename)
		srcfolder = self._storage(FileDestinations.USB).basefolder
		dstfolder = self._storage(FileDestinations.LOCAL).basefolder
		srcfile = os.path.join(srcfolder, filename)
		count = 0
		tmpfilename = filename
		while self.file_exists(FileDestinations.LOCAL, tmpfilename):
			count += 1
			tmpfilename = "cp" + str(count) + "_" + filename
		filename = tmpfilename
		dstfile = os.path.join(dstfolder, filename)
		copyer = threading.Thread(target=self.copyFile,args=(dstfile, srcfile))
		copyer.daemon = True
		copyer.start()
		copyer.join(timeout)
		return filename
	
	def copyFile(self, dstFile, srcFile):
		filesize = os.path.getsize(srcFile)
		readsize = 1024*1024
		rwtimes = round(filesize/readsize) + 1
		rwcount = 0.0
		with open(srcFile, 'rb') as fsrc:
			with open(dstFile, 'wb') as fdst:
				while True:
					rwcount += 1
					self._copyPercent = round((rwcount/rwtimes)*100)
					readbuf = fsrc.read(readsize)
					if not readbuf:
						self._copyPercent = 100
						self._logger.info("[%s] has been copy to [%s]!" %(srcFile, dstFile))
						break
					fdst.write(readbuf)
		sleep(0.1)
	# zyd for usb storage -e
