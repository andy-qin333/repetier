...
from octoprint.util import InvariantContainer, to_str
...
class Printer(PrinterInterface, comm.MachineComPrintCallback):
	...
	def jogInverted(self, axis, amount):
		if not isinstance(axis, (str, unicode)):
			raise ValueError("axis must be a string: {axis}".format(axis=axis))
	...
	def set_temperature(self, heater, value):
		if not PrinterInterface.valid_heater_regex.match(heater):
			raise ValueError("heater must match \"tool[0-9]+\" or \"bed\" or \"chamber\": {heater}".format(type=heater))

		#if not isinstance(value, (int, long, float)) or value < 0:
		if not isinstance(value, (int, long, float)): # add by evan, for modify temperature
	...
	def select_file(self, path, sd, printAfterSelect=False, recoverPrint=False, pos=None):
		path = to_str(path, encoding='utf-8')
		...
		if recovery_data:
			...
			actual_origin = to_str(recovery_data.get("origin", None), encoding='utf-8')
			actual_path = to_str(recovery_data.get("path", None), encoding='utf-8')
		...
		# zyd for continue print when print job aborted -s
		self._recoverPrint = recoverPrint
		# zyd for continue print when print job aborted -e
	...
	def start_print(self, pos=None, recoverPrint=False):
		"""
		...
		self._setProgressData(completion=0)
		self._setCurrentZ(None)
		self._comm.startPrint(pos=pos, recoverPrint=recoverPrint)
	...
	# zyd for ex_state -s
	def set_bed_level(self, mode):
		self._comm.setBedLevel(mode)
		
	def set_material_blocked(self, mode, num):
		self._comm.setMaterialBlocked(mode, num)
		
	def set_material_lost(self, mode, num): # add by evan, for material-lost
		self._comm.setMaterialLost(mode, num)
	# zyd for ex_state -e
	# add by evan, for pause-tip -s
	def set_handle_tip(self, mode):
		self._comm.setHandleTip(mode)
	# add by evan, for pause-tip -e
	
	# add by evan, for process the problem while the machine is in the state of connecting -s
	def set_connecting_state(self, mode):
		self._comm.setConnectingState(mode)
	...
	def set_error_message_status(self, state):
		self._comm.setErrorMessageStatus(state)
	# add by evan, for show error message -e
	# zyd for power off -s
	def start_power_off(self):
		self._comm.startPowerOff()
		
	def set_auto_power_off(self, mode):
		self._comm.setAutoPowerOff(mode)
		
	def set_request_power_off(self, mode):
		self._comm.setRequestPowerOff(mode)
	# zyd for power off -e
	...
	# zyd add for ex_state -s
	def get_bed_level_state(self):
		if self._comm is None:
			return 0
		else:
			return self._comm.getBedLevelState()
	
	def get_material_blocked(self, num):
		return self._comm is not None and self._comm.getMaterialBlocked(num)
	
	def get_material_lost(self, num): # add by evan, for material-lost
		return self._comm is not None and self._comm.getMaterialLost(num)	
	# zyd add for ex_state -e
	
	# add by evan, for pause-tip -s
	def get_handle_tip(self):
		return self._comm is not None and self._comm.getHandleTip()
	# add by evan, for pause-tip -e
	# add by evan, for process the problem while the machine is in the state of connecting -s
	def get_connecting_state(self):
		return self._comm is not None and self._comm.getConnectingState()
	# add by evan, for process the problem while the machine is in the state of connecting -e

	# zyd for power off -s
	def is_auto_power_off(self):
		return self._comm is not None and self._comm.isAutoPowerOff()
	
	def is_request_power_off(self):
		return self._comm is not None and self._comm.isRequestPowerOff()
	# zyd for power off -e
	
	# zyd for continue print when print job aborted -s
	def is_print_job_aborted(self):
		return self._comm is not None and self._comm.isPrintJobAborted()
	# zyd for continue print when print job aborted -e
	
	# zyd for get about information -s
	def get_about_information(self, sub):
		if self._comm is None:
			return None
		return self._comm.getAboutInformation(sub)
	...
	def get_Error_Message(self):
		if self._comm is None:
			return None
		return self._comm.getErrorMsg()
	def get_error_message_status(self):
		return self._comm is not None and self._comm.getErrorMsgStatus()
	# add by evan, for show error message -e	
	...
	def get_remaining_info(self):
		if self._comm is None:
			return None
		return self._comm.getRemainingInfo()
	...
	def _addTemperatureData(self, temp, bedTemp, chamberTemp):
		...
		# zyd for add chamber -s
		if chamberTemp is not None and isinstance(chamberTemp, tuple):
			data["chamber"] = {
			        "actual": chamberTemp[0],
			        "target": chamberTemp[1]
			}
		# zyd for add chamber -e
	...
	def _setJobData(self, filename, filesize, sd):
		if filename is not None:
			if sd:
				...
				path_on_disk = to_str(path_on_disk, encoding='utf-8')
			self._selectedFile = {
	...
	# zyd for ex_state -s
	def _getExState(self):
		return {
		        "bedLevelState": self.get_bed_level_state(),
		        "materialBlocked0": self.get_material_blocked(0),
		        "materialBlocked1": self.get_material_blocked(1),
		        "materialLost0": self.get_material_lost(0), # add by evan, for material-lost
		        "materialLost1": self.get_material_lost(1), # add by evan, for material-lost
		        "handleTip": self.get_handle_tip(), # add by evan, for pause-tip
		        "connectingState": self.get_connecting_state(),# add by evan, for process the problem while the machine is in the state of connecting
		        # zyd for power off -s
		        "autoPowerOff": self.is_auto_power_off(),
		        "requestPowerOff": self.is_request_power_off(),
		        "printJobAborted": self.is_print_job_aborted(),
		        # zyd for power off -e
		        # zyd for get about information -s
		        "serialNumber": self.get_about_information("sn"),
		        "hardwareVersion" : self.get_about_information("hv"),
		        "firmwareVersion" : self.get_about_information("sv"),
		        "module" : self.get_about_information("mv"),
		        "printedLength" : self.get_about_information("pl"),
		        "softwareVersion" : self.get_about_information("ss"),
		        # zyd for get about information -e
		        
		        # add by evan, for remaining-material -s
		        "remainingMaterial":self.get_remaining_info(),
		        # add by evan, for remaining-material -e
		        "showErrorMessag":self.get_Error_Message()
		}
	# zyd for ex_state -e
	...
	def on_comm_temperature_update(self, temp, bedTemp, chamberTemp):
		self._addTemperatureData(temp, bedTemp, chamberTemp)
	...
	def on_comm_file_selected(self, filename, filesize, sd):
		...
		if self._printAfterSelect:
			self.start_print(pos=self._posAfterSelect, recoverPrint=self._recoverPrint)
	...
	def on_comm_ex_state_change(self):
		self._stateMonitor.set_ex_state(self._getExState())
	# zyd for ex_state -e
...
class StateMonitor(object):
	def __init__(self, interval=0.5, on_update=None, on_add_temperature=None, on_add_log=None, on_add_message=None, on_get_progress=None):
		...
		#zyd for ex_state -s
		self._ex_state = None
		self._ex_state_lock = threading.Lock()
		#zyd for ex_state -e
	...
	# zyd for ex_state -s
	def _get_ex_state(self):
		with self._ex_state_lock:
			return self._ex_state
	# zyd for ex_state -e

	def reset(self, state=None, job_data=None, progress=None, current_z=None, exState=None):
		self.set_state(state)
		self.set_job_data(job_data)
		self.set_progress(progress)
		self.set_current_z(current_z)
		self.set_ex_state(exState)
	...
	def get_current_data(self):
		with self._progress_lock:
			if self._progress_dirty:
				...
				self._progress_dirty = False
		#zyd for ex_state -s
		self._ex_state = self._get_ex_state()
		#zyd for ex_state -e
		return {
			"state": self._state,
			"job": self._job_data,
