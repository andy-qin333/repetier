...
from octoprint.util import get_exception_string, sanitize_ascii, filter_non_ascii, CountedEvent, RepeatedTimer, \
	to_unicode, to_str, bom_aware_open, TypedQueue, TypeAlreadyInQueue
...
regex_positive_float_pattern = "[-+]?[0-9]*\.?[0-9]+" # add by evan, for modify temperature
regex_int_pattern = "\d+"
...
regex_temp = re.compile("(?P<tool>B|C|T(?P<toolnum>\d*)):\s*(?P<actual>%s)(\s*\/?\s*(?P<target>%s))?" % (regex_positive_float_pattern, regex_positive_float_pattern))
...
regex_repetierTempChamber = re.compile("TargetChamber:(?P<target>%s)" % regex_positive_float_pattern)
# zyd for add chamber -e

regex_pos_z = re.compile(".*Z:?(%s)\s*" %regex_float_pattern)
regex_ActiveExtruder = re.compile(".*Active\sExtruder:\s([0-9])\s*")
#regex_AutoLevel = re.compile(".*AutoLevel:([0-9])\s*")
regex_AutoLevel = re.compile(".*AutoLevel:(.*)")

# zyd for get about information -s
regex_v_sn = re.compile("V-Serial\snumber:([A-Z][A-Z\d]\d{8})")
regex_v_mn = re.compile("V-Module:(.*)")
regex_v_hv = re.compile("V-Hardware\sversion:V(\d.\d{2})")
regex_v_sv = re.compile("V-Firmware\sversion:V(\d.\d{3})")
# zyd for get about information -e

# add by evan, for remaining-material -s
regex_rm = re.compile(".*Remaining:(.*)m")
...
class MachineCom(object):
	...
	# zyd for ex_state -s
	LEVEL_STATE_NONE = 0
	LEVEL_STATE_FINISHED = 1
	LEVEL_STATE_FAILED = 2
	# zyd for ex_state -e
	...
	def __init__(self, port = None, baudrate=None, callbackObject=None, printerProfileManager=None):
		...
		self._print_job_aborted = False
		# zyd for continue print when print job aborted -e
		# zyd for ex_state -s
		self._bedLevelState = self.LEVEL_STATE_NONE
		self._materialBlocked0 = False
		self._materialBlocked1 = False
		self._materialLost0 = False # add by evan, for material-lost
		self._materialLost1 = False # add by evan, for material-lost
		self._handleTip = False # add by evan, for pause-tip
		self._connectingState = False # add by evan, for process the problem while the machine is in the state of connecting
		# zyd for ex_state -e
		# zyd for power off -s
		self._autoPowerOff = False
		self._requestPowerOff = False
		# zyd for power off -e
		# zyd for get about information -s
		self._aboutInformation = {"sn": None, "hv": None, "sv": None, "mv": None, "pl":None, "ss":octoprint._version.get_versions()["version"]}
		# zyd for get about information -e
		
		# add by evan, for show error message -s
		self._showErrorMsg = None
		self._showErrorMsgStatus = False
		# add by evan, for show error message -e
		
		# add by evan, for remaining-material -s
		self._remainingInfo = None
		# add by evan, for remaining-material -e
	...
	def _changeState(self, newState):
		...
		if oldState is "Printing" and self.getStateString() is "Operational":
			self.sendCommand("M268 S0")
		elif oldState is "Operational" and self.getStateString() is "Printing":
			self.sendCommand("M268 S1")
		elif oldState is "Paused" and self.getStateString() is "Printing":
			self.sendCommand("M268 S1")
			pass
	...
	# zyd for ex_state -s
	def getBedLevelState(self):
		return self._bedLevelState
		
	def getMaterialBlocked(self, num):
		if num is 0:
			return self._materialBlocked0
		elif num is 1:
			return self._materialBlocked1
		
	def getMaterialLost(self, num): # add by evan, for material-lost
		if num is 0:
			return self._materialLost0
		elif num is 1:
			return self._materialLost1
	# zyd for ex_state -e
	
	# add by evan, for pause-tip -s
	def getHandleTip(self):
		return self._handleTip
	# add by evan, for pause-tip -e

	# add by evan, for process the problem while the machine is in the state of connecting -s
	def getConnectingState(self):
		return self._connectingState
	# add by evan, for process the problem while the machine is in the state of connecting -e
	
	# zyd for power off -s
	def isAutoPowerOff(self):
		return self._autoPowerOff
	
	def isRequestPowerOff(self):
		return self._requestPowerOff
	# zyd for power off -e
	
	# zyd for continue print when print job aborted -s
	def isPrintJobAborted(self):
		return self._print_job_aborted
	# zyd for continue print when print job aborted -e
	
	# zyd for get about information -s
	def getAboutInformation(self, sub):
		return self._aboutInformation[sub]
	# zyd for get about information -e
	
	# add by evan, for show error message -s	
	def getErrorMsg(self):
		return self._showErrorMsg
	def getErrorMsgStatus(self):
		return self._showErrorMsgStatus
	# add by evan, for show error message -e	
	
	# add by evan, for remaining-material -s
	def getRemainingInfo(self):
		return self._remainingInfo
	# add by evan, for remaining-material -e
	##~~ external interface
	...
	def startPrint(self, pos=None, recoverPrint=False):
		if self.isPrintJobAborted():
			self.sendCommand("M13")
		#self.sendCommand("M265 S1") # add by evan, for door-lock
		...
		try:
			...
			if self.isSdFileSelected():
				...
				# zyd for continue print when print job aborted -s
				if recoverPrint:
					self.sendCommand("M13")	# get record position
				else:
					self.sendCommand("M14 S1") # active record position
					self.setPrintJobAborted(False)
					self._changeState(self.STATE_PRINTING)
					self.resetLineNumbers()
					line = self._getNext()
					if line is not None:
						self.sendCommand(line)
				settings().set(["printerParameters","lastFileName"],self._currentFile.getFilename(),True)
				settings().save()
				# zyd for continue print when print job aborted -e
	...
	def cancelPrint(self, firmware_error=None):
		...
		self.sendCommand("M29")    # zyd add for cancel print quickly
		self.sendCommand("M14 S0")#deactivate record position
		# zyd for get about information -s
		self.sendCommand("M205 P376")
		# zyd for get about information -e
		# zyd for continue print when print job aborted -s
		if self.isPrintJobAborted() == False:
			settings().set(["printerParameters", "lastFileName"], None, force=True)
			settings().set(["printerParameters", "filePosition"], 0, force=True)
			settings().save()
		# zyd for continue print when print job aborted -e
		
		# zyd add for go down platform as cancel print job -s
		self._callback.commands(["G90","G1 Z%s F1000" %str(self._printerProfileManager.get_current_or_default()["volume"]["height"])])
	...
	def setPause(self, pause):
		...
		if not pause and self.isPaused():
			if self._print_job_aborted is True:
				self.sendCommand("M15 S1")
			...
			if self.isSdFileSelected():
				...
				lastTool = settings().get(["printerParameters", "lastTool"])
				self._callback.change_tool("tool%s" %lastTool)
				usedTools = settings().get(["printerParameters", "usedTools"])
				for usedTool in usedTools:
					if usedTool[0] + 1 == len(usedTools):
						self.sendCommand("M109 T%s S%s" %(str(usedTool[0]), str(usedTool[1])))
					else:
						self.sendCommand("M104 T%s S%s" %(str(usedTool[0]), str(usedTool[1])))
				self._callback.commands(["G91", "G1 E10 F300", "G1 E-5 F300", "G1 E5 F300", "G90"]) # for cut silk
				self.sendCommand("M26")
			...
			if self.isSdFileSelected():
				...
				usedTools = settings().get(["printerParameters", "usedTools"])
				for usedTool in usedTools:
					self.sendCommand("M104 T%s S170" %str(usedTool[0]))
				self.sendCommand("M25") # pause print then get printer position
	...
	# zyd for ex_state -s
	def setBedLevel(self, mode):
		self._bedLevelState = mode
		self._callback.on_comm_ex_state_change()

	def setMaterialBlocked(self, mode, num):
		if num is 0:
			self._materialBlocked0 = mode
		elif num is 1:
			self._materialBlocked1 = mode
		self._callback.on_comm_ex_state_change()

	def setMaterialLost(self, mode, num): # add by evan, for material-lost
		if num is 0:
			self._materialLost0 = mode
		elif num is 1:
			self._materialLost1 = mode
		self._callback.on_comm_ex_state_change()		
	# zyd for ex_state -e
	
	# add by evan, for pause-tip -s
	def setHandleTip(self, mode):
		self._handleTip = mode
		self._callback.on_comm_ex_state_change()	
	# add by evan, for pause-tip -e
	
	# add by evan, for process the problem while the machine is in the state of connecting -s 
	def setConnectingState(self, mode):
		self._connectingState = mode
		self._callback.on_comm_ex_state_change()
	# add by evan, for process the problem while the machine is in the state of connecting -e 

	# zyd for power off -s
	def startPowerOff(self):
		self._callback.commands(["G28 Z0", "M1"])
	
	def setAutoPowerOff(self, mode):
		self._autoPowerOff = mode
		self._callback.on_comm_ex_state_change()
		
	def setRequestPowerOff(self, mode):
		self._requestPowerOff = mode
		self._callback.on_comm_ex_state_change()
	# zyd for power off -e
	
	# zyd for continue print when print job aborted -s
	def setPrintJobAborted(self, mode):
		self._print_job_aborted = mode
		self._callback.on_comm_ex_state_change()
	# zyd for continue print when print job aborted -e
	
	# zyd for get about information -s
	def setAboutInformation(self, sub, info):
		self._aboutInformation[sub] = info
		self._callback.on_comm_ex_state_change()
	# zyd for get about information -e
	
	# add by evan, for show error message -s
	def setErrorMessage(self, info):
		self._showErrorMsg = info
		self._callback.on_comm_ex_state_change()
	def setErrorMessageStatus(self, state):
		self._showErrorMsgStatus = state
		self._callback.on_comm_ex_state_change()
	# add by evan, for show error message -e
	
	# add by evan, for remaining-material -s
	def setRemainingInfo(self, info):
		self._remainingInfo = info
		self._callback.on_comm_ex_state_change()
	# add by evan, for remaining-material -e
	def getSdFiles(self):
	...
	# zyd for continue print when print job aborted -s
	def _recoverLastPrintInformation(self, line):
		regex_abort_info = re.compile(".*X:(%s)\s*Y:(%s)\s*Z:(%s)\s*E:(%s)\s*TE:(%s)\s*" %(regex_float_pattern,regex_float_pattern,regex_float_pattern,regex_float_pattern,regex_int_pattern))
		abort_info = re.match(regex_abort_info, line)
		flag = 0
		if self._currentFile is None or abort_info is None:
			return None
		try:
			x_pos = float(abort_info.group(1))
			y_pos = float(abort_info.group(2))
			z_pos = float(abort_info.group(3))
			e_pos = float(abort_info.group(4))
			which_tool = int(abort_info.group(5))
			file_pos = long(settings().get(["printerParameters", "filePosition"]))
			
			lastFileName = settings().get(["printerParameters","lastFileName"])
			#lastFileName = to_str(lastFileName)
			lastFilename_content = open(lastFileName)
			try:
				all_the_text = lastFilename_content.read( )
			except:
				pass
			
			if "M140" in all_the_text:
				regex_bedTmp = re.compile("M140 S(.*)")
				bed_temp = re.search(regex_bedTmp, all_the_text)
				flag = 140
			if "M190" in all_the_text:
				regex_bedTmp = re.compile("M190 S(.*)")
				bed_temp = re.search(regex_bedTmp, all_the_text)
				flag = 190
		...
		#command_list = ["G21", "G90", "G28"]
		command_list = ["G21", "G90"]
			
		if flag is 140:
			command_list.extend(["M140 S%s" % bed_temp.group(1)])
		elif flag is 190:
			command_list.extend(["M190 S%s" % bed_temp.group(1)])
			
		#parse file header
		file_header_line_num = 20
		regex_set_temp = re.compile(".*T\d\s*S\d+")
		regex_set_temp1 = re.compile("M104 S\d+")
		regex_set_temp2 = re.compile("M109 S\d+")
		while (file_header_line_num):
			file_header_line_num -= 1
			line = self._currentFile.getLine()
			if line is None:
				return None
			if (re.match(regex_set_temp, line) is not None):
				command_list.append(line)
			if (re.match(regex_set_temp1, line) is not None):
				command_list.append(line)
			if (re.match(regex_set_temp2, line) is not None):
				command_list.append(line)
		command_list.append("G28") # add by evan, for add tool temperature after move
		
		is_found_XY = False
		is_found_Z = False
		increment = 0
		self._currentFile.seek(file_pos)
		while (is_found_XY is False or is_found_Z is False):
			line = self._currentFile.getLine()
			if line is None:
				return None
			increment += len(line)
			if "G0" in line or "G1" in line:
				if is_found_XY == False:
					if "X%.3f Y%.3f"%(x_pos, y_pos) in line:
						is_found_XY = True
						file_pos += increment
						increment = 0
				if is_found_Z == False:
					if "Z" in line:
						if "X%.3f Y%.3f Z%.3f"%(x_pos, y_pos, z_pos) in line:
							is_found_Z = True
						else:
							pos_z = re.match(regex_pos_z, line)
							if pos_z is not None:
								try :
									if float(pos_z.group(1)) > z_pos:
										if is_found_XY:
											is_found_Z = True
									else:
										is_found_XY = False
		...
		command_list.extend(["T0", "G92 E0"])
		command_list.extend(["T1", "G92 E0"])
		command_list.extend(["T%s" %which_tool, "G1 E10 F300", "G1 E-5 F300", "G1 E5 F300", "G92 E%s" %e_pos])
		command_list.extend(["G90", "G1 X%s Y%s Z%s F3000" %(x_pos, y_pos, z_pos)])
		command_list.extend(["M106 S255"])
	...
	def _processTemperatures(self, line):
		...
		# zyd for add chamber -s
		if "C" in parsedTemps.keys():
			actual, target = parsedTemps["C"]
			if target is not None:
				self._chamberTemp = (actual, target)
			elif self._chamberTemp is not None and isinstance(self._chamberTemp, tuple):
				(oldActual, oldTarget) = self._chamberTemp
				self._chamberTemp = (actual, oldTarget)
			else:
				self._chamberTemp = (actual, None)
		# zyd for add chamber -e
	...
	def _monitor(self):
		...
		while self._monitoring_active:
			try:
				...
				# zyd for continue print when print job aborted -s
				if 'X:' in line and 'Y:' in line and 'Z:' in line and 'E' in line and 'TE:' in line:
					if self.isPrintJobAborted():
						self.setPrintJobAborted(False)
						recover_commands = self._recoverLastPrintInformation(line)
						if recover_commands is not None:
							self._changeState(self.STATE_PRINTING)
							self.resetLineNumbers()
							self._callback.commands(recover_commands)
						else:
							self._currentFile.seek(0)
							self._changeState(self.STATE_PRINTING)
							self.resetLineNumbers()
						self.sendCommand("M14 S1")
						line = self._getNext()
						if line is not None:
							self.sendCommand(line)
						self._sendFromQueue()
				...
				# position report processing
					...
					# see also issue #1373
					pass
				# zyd for auto bed level -s
				elif line.startswith("AutoLevel:"):
					matchAutoLevel = re.match(regex_AutoLevel, line)
					if matchAutoLevel is not None:
						try:
							levelResult = matchAutoLevel.group(1)
							if levelResult == "0":
								self._bedLevelState = self.LEVEL_STATE_FAILED
							else:
								self._bedLevelState = self.LEVEL_STATE_FINISHED
							self.setBedLevel(self._bedLevelState)
						except ValueError:
							pass
				# zyd for auto bed level -e
				
				# add by evan, for remaining-material -s
				elif line.startswith("Remaining:"):
					matchRemaining = re.match(regex_rm, line)
					if matchRemaining is not None:
						try:
							remainingResult = matchRemaining.group(1)
							self.setRemainingInfo(remainingResult)
						except ValueError:
							pass
				# add by evan, for remaining-material -e
				
				# zyd for material block -s
				elif line.startswith("extr.0 feed blockage"):
					self.setMaterialBlocked(True, 0)
					self._print_job_aborted = True
					self.setPause(True)
				elif line.startswith("extr.1 feed blockage"):
					self.setMaterialBlocked(True, 1)
					self._print_job_aborted = True
					self.setPause(True)
				# zyd for material block -e
				elif line.startswith("extr.0 feed empty"): # add by evan, for material-lost
					self.setMaterialLost(True, 0)
					self._print_job_aborted = True
					self.setPause(True)
				elif line.startswith("extr.1 feed empty"): # add by evan, for material-lost
					self.setMaterialLost(True, 1)
					self._print_job_aborted = True
					self.setPause(True)
				# add by evan, for pause-tip -s
				elif "print paused" in line or "print canceled" in line:
					self.setHandleTip(True)
				# add by evan, for pause-tip -e
				# zyd for power off -s
				elif line.startswith("PowerOff"):
					self.setRequestPowerOff(True)
				# zyd for power off -e
				# zyd for get about information -s
				elif line.startswith("V-"):
					if "Serial number:" in line:
						sn = re.match(regex_v_sn, line)
						if sn is not None:
							self.setAboutInformation("sn", sn.group(1))
							settings().set(["printerParameters", "serialNumber"], sn.group(1), force=True)
							self.setConnectingState(True)
							settings().save()
					elif "Hardware version:" in line:
						hv = re.match(regex_v_hv, line)
						if hv is not None:
							self.setAboutInformation("hv", hv.group(1))
					elif "Firmware version:" in line:
						sv = re.match(regex_v_sv, line)
						if sv is not None:
							self.setAboutInformation("sv", sv.group(1))
					elif "Module:" in line:
						mv = re.match(regex_v_mn, line)
						if mv is not None:
							self.setAboutInformation("mv", mv.group(1))
				elif line.startswith("addr:376"):
					regex_printed_length = re.compile("addr:376\stype:1.*\s(%s)\s*" %regex_float_pattern)
					pl = re.match(regex_printed_length, line)
					if pl is not None:
						self.setAboutInformation("pl", float(pl.group(1)))
				# zyd for get about information -e

				# temperature processing
				elif ' T:' in line or line.startswith('T:') or ' T0:' in line or line.startswith('T0:') or ((' B:' in line or line.startswith('B:') or ' C:' in line or line.startswith('C:')) and not 'A:' in line):
					...
					self._callback.on_comm_temperature_update(self._temp, self._bedTemp, self._chamberTemp)

				elif supportRepetierTargetTemp and ('TargetExtr' in line or 'TargetBed' in line or 'TargetChamber' in line):
					matchExtr = regex_repetierTempExtr.match(line)
					matchBed = regex_repetierTempBed.match(line)
					# zyd for add chamber -s
					matchChamber = regex_repetierTempChamber.match(line)
					# zyd for add chamber -e
					...
					if matchExtr is not None:
						...
						try:
							...
							# zyd for pause and resume -s
							if self.isPrinting():
								usedTools = []
								if toolNum < 2 and target not in (0, 0.0, None):
									usedTools.append((toolNum, target))
								settings().set(["printerParameters", "usedTools"], usedTools, force=True)
							# zyd for pause and resume -e
						except ValueError:
							pass
						...
						try:
							...
							pass
					# zyd for add chamber -s
					elif matchChamber is not None:
						try:
							target = float(matchChamber.group(1))
							if self._chamberTemp is not None and isinstance(self._chamberTemp, tuple):
								(actual, oldTarget) = self._chamberTemp
								self._chamberTemp = (actual, target)
							else:
								self._chamberTemp = (None, target)
							self._callback.on_comm_temperature_update(self._temp, self._chamberTemp, self._chamberTemp)
						except ValueError:
							pass
					# zyd for add chamber -e
						
				# zyd for pause and resume -s
				elif 'Active Extruder' in line:
					if self.isPrinting():
						matchTool = re.match(regex_ActiveExtruder, line)
						if matchTool is not None:
							lastTool = int(matchTool.group(1))
							settings().set(["printerParameters", "lastTool"], lastTool)
				# zyd for pause and resume -e
				...
				### Connection attempt
					...
					if "start" in line and not startSeen:
						...
						self._onConnected()
						# zyd for get about information -s
						self._sendCommand("V0;")
						self._sendCommand("V2;")
						self._sendCommand("V4;")
						self._sendCommand("V6;")
						self._sendCommand("M205 P376")
						# zyd for get about information -e
					elif time.time() > self._timeout:
						self.close()
					# zyd for match M3 board -s
					elif "Resend:" in line or "unknown command" in line:
						self.sayHello()
					elif "unknown command" in line:
						print "connecting unknown command"
						self._log("connecting unknown command")
					# zyd for match M3 board -e
	...
	def _poll_temperature(self):
		...
		if self.isOperational() and not self._connection_closing and not self.isStreaming() and not self._long_running_command and not self._manualStreaming:#zyd notice
	...
	def _handleErrors(self, line):
		# add by evan, for show error message -s
		errorInfo = re.match(regex_error, line)	
		if errorInfo is not None:
			if "temperature sensor" in errorInfo.group(1):
				print "temperature sensor is invalid"
				self.setErrorMessage("error:"+errorInfo.group(1))
				self.setErrorMessageStatus(True)
		# add by evan, for show error message -e
		...
		if lower_line.startswith('error:') or line.startswith('!!'):
			...
			if 'Bootloader' in lower_line:
					print "error!!!"
			elif 'line number' in lower_line or 'checksum' in lower_line or 'format error' in lower_line or 'expected line' in lower_line:
				self.sendCommand("M268 S3")
				...
				pass
			elif 'extr.0 feed blockage' in lower_line:
				self.setMaterialBlocked(True, 0)
				self._print_job_aborted = True
				self.setPause(True)
			elif 'extr.1 feed blockage' in lower_line:
				self.setMaterialBlocked(True, 1)
				self._print_job_aborted = True
				self.setPause(True)
			elif 'extr.0 feed empty' in lower_line:
				self.setMaterialLost(True, 0)
				self._print_job_aborted = True
				self.setPause(True)
			elif 'extr.1 feed empty' in lower_line:
				self.setMaterialLost(True, 1)
				self._print_job_aborted = True
				self.setPause(True)
				...
				pass
			# add by evan, for reconnect -s
			elif 'unknow cmd-0' in lower_line:
				try:
					self._log("____reconnect____")
					if not self._openSerial():
						return
				except:
					self._log("____reconnect error____")
					pass
			# add by evan, for reconnect -e
	...
	# zyd for add chamber -s
	def _gcode_M141_queuing(self, cmd, cmd_type=None):
	...
	# zyd for add chamber -s
	def _gcode_M141_sent(self, cmd, cmd_type=None, wait=False):
		match = regexes_parameters["floatS"].search(cmd)
		if match:
			try:
				target = float(match.group("value"))
	...
	# zyd for add chamber -s
	def _gcode_M191_sent(self, cmd, cmd_type=None):
		self._heatupWaitStartTime = time.time()
		self._long_running_command = True
		self._heating = True
		self._gcode_M141_sent(cmd, cmd_type, wait=True)
	# zyd for add chamber -e
	...
	def _gcode_M112_queuing(self, cmd, cmd_type=None):
		...
		# zyd for add chamber -s
		if self._printerProfileManager.get_current_or_default()["heatedChamber"]:
			self._do_increment_and_send_with_checksum("M141 S0")
		# zyd for add chamber -e
...
class MachineComPrintCallback(object):
	...
	# zyd for ex_state -s
	def on_comm_ex_state_change(self):
		pass
	# zyd for ex_state -e
...
class PrintingGcodeFileInformation(PrintingFileInformation):
	...
	def __init__(self, filename, offsets_callback=None, current_tool_callback=None):
		...
		self._filename=to_str(self._filename, encoding='utf-8')
	...
	def getNext(self):
		...
		try:
			...
			self._read_lines += 1
			# zyd for continue print when print job aborted -s
			if self._read_lines % 10000 == 0 or self._read_lines == 1:
				if self._pos > 2000:
					settings().set(["printerParameters", "filePosition"], self._pos - 2000)
					settings().save()
			# zyd for continue print when print job aborted -e
			return processed
	...
	def getLine(self):
		...
		try:
			...
			self.close()
			self._logger.exception("Exception while getting line")
			raise e
	# zyd for continue print when print job aborted -e
...
def apply_temperature_offsets(line, offsets, current_tool=None):
	...
	# zyd for add chamber -s
	elif groups["command"] == "141" or groups["command"] == "191":
		# chamber temperature
