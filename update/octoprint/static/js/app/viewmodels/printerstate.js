$(function() {
    function PrinterStateViewModel(parameters) {
        ...
        // zyd for power off -s
        self.enableAutoPowerOff = ko.pureComputed(function() {
            return self.isOperational() && (self.isPrinting() || self.isPaused()) && self.loginState.isUser();
        });
        // zyd for power off -e
        ...
        // zyd for power off -s
        self.autoPowerOff = ko.observable(false);
        // zyd for power off -e
        ...
        self._fromData = function(data) {
            ...
            // zyd for power off -s
            self._processExStateData(data.exState);
            // zyd for power off -e
        };
        ...
        // zyd for power off -s
        self._processExStateData = function(data) {
            self.autoPowerOff(data.autoPowerOff);
        };
        
        self.start_power_off = function() {
            var data = {
                "command": "powerOff",
            };
            self._jobExCommand(data);
        };
        
        self.setAutoPowerOff = function() {
            var data = {
                "command": "autoPowerOff",
                "mode": !self.autoPowerOff()
            };
            self._jobExCommand(data);
        };
        
        self._jobExCommand = function(data, callback) {
            $.ajax({
                url: API_BASEURL + "job/ex",
                type: "POST",
                dataType: "json",
                contentType: "application/json; charset=UTF-8",
