...
class VirtualPrinter():
	...
	def __init__(self, read_timeout=5.0, write_timeout=10.0):
		...
		self.chamberTemp = 1.0
		self.chamberTargetTemp = 1.0
	...
	def _processIncoming(self):
		...
		while self.incoming is not None and not self._killed:
			...
			# zyd for add chamber -s
			if 'M141' in data or 'M191' in data:
			...
			# zyd for continue print when print job aborted -s
				...
				output = "X:146.766 Y:114.805 Z:25.100 E:213.97286 TE:0"
				self.outgoing.put(output)
			# zyd for continue print when print job aborted -e
			# zyd for get about information -s
			elif "M205 P376" in data:
				self.outgoing.put("addr:376 type:1(float) 100.0000 Filament printed[mm]")
			...
			# zyd for get about information -s
				...
				self.outgoing.put("V-Serial number:B212345678")
			elif "V4;" in data:
				self.outgoing.put("V-Hardware version:1.00")
			elif "V6;" in data:
				self.outgoing.put("V-Firmware version:1.000")
			elif "RM;" in data:
				self.outgoing.put("Remaining:123.4m")
			# zyd for get about information -e
	...
	def _debugTrigger(self, data):
		...
		# zyd for virtual debug -s
		elif data == "PowerOff":
			self.outgoing.put("PowerOff")
		elif data == "extr.0 feed blockage":
			self.outgoing.put("extr.0 feed blockage")
		elif data == "extr.1 feed blockage":
			self.outgoing.put("extr.1 feed blockage")
		elif data == "extr.0 feed empty": # add by evan, for material-lost
			self.outgoing.put("extr.0 feed empty")
		elif data == "extr.1 feed empty": # add by evan, for material-lost
			self.outgoing.put("extr.1 feed empty")
		elif data == "AutoLevel:0":
			self.outgoing.put("AutoLevel:0")
		elif data == "AutoLevel:1":
			self.outgoing.put("AutoLevel:1")
		elif data == "Remaining:":
			self.outgoing.put("Remaining:102.3m")
		# zyd for virtual debug -e
		else:
			try:
	...
	def _processTemperatureQuery(self):
		...
		if settings().getInt(["devel", "virtualPrinter", "numExtruders"]) > 1:
			...
			if includeTarget:
				output = "T:%.2f /%.2f B:%.2f /%.2f C:%.2f /%.2f @:64\n" % (self.temp[0], self.targetTemp[0], self.bedTemp, self.bedTargetTemp, self.chamberTemp, self.chamberTargetTemp)
			else:
				output = "T:%.2f B:%.2f C:%.2f @:64\n" % (self.temp[0], self.bedTemp, self.chamberTemp)
	...
	# zyd for add chamber -s
	def _parseChamberCommand(self, line):
		try:
		...
		if "M191" in line:
			self._waitForHeatup("chamber")
		if settings().getBoolean(["devel", "virtualPrinter", "repetierStyleTargetTemperature"]):
			self.outgoing.put("TargetChamber:%d" % self.chamberTargetTemp)
	# zyd for add chamber -e
		...
		try:
			with open(self._selectedSdFile, "r") as f:
				for line in iter(f.readline, ""):
					...
					# zyd for add chamber -s
					elif 'M141' in line or 'M191' in line:
						self._parseChamberCommand(line)
					# zyd for add chamber -e
	...
	def _waitForHeatup(self, heater):
		...
		try:
			...
			# zyd for add chamber -s
				...
				while not self._killed and (self.chamberTemp < self.chamberTargetTemp - delta or self.chamberTemp > self.chamberTargetTemp + delta):
					self._simulateTemps(delta=delta)
					self.outgoing.put("B:%0.2f" % self.chamberTemp)
					time.sleep(delay)
			# zyd for add chamber -e
		except AttributeError:
			if self.outgoing is not None:
	...
	def _simulateTemps(self, delta=1):
		...
		if abs(self.bedTemp - self.bedTargetTemp) > delta:
			...
			if self.bedTemp < 0:
				self.bedTemp = 0
		# zyd for add chamber -s
		if abs(self.chamberTemp - self.chamberTargetTemp) > delta:
			oldVal = self.chamberTemp
			self.chamberTemp += math.copysign(timeDiff * 10, self.chamberTargetTemp - self.chamberTemp)
			if math.copysign(1, self.chamberTargetTemp - oldVal) != math.copysign(1, self.chamberTargetTemp - self.chamberTemp):
				self.chamberTemp = self.chamberTargetTemp
			if self.chamberTemp < 0:
				self.chamberTemp = 0
		# zyd for add chamber -e
