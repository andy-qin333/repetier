...
defaults = dict(
    ...
    print_chamber_temperature=0, # zyd for add chamber
    ...
    has_heated_chamber=False, # zyd for add chamber
...
class Profile(object):
	...
	def get(self, key):
		...
		elif key == "has_heated_chamber":
			return self._printer_profile["heatedChamber"]
	...
	def get_gcode(self, key):
		...
		if key == "start_gcode":
			...
			# zyd for add chamber -s
			chamber_temp = 0
			if self.get_boolean("has_heated_chamber"):
				chamber_temp = self.get_float("print_chamber_temperature")
			include_chamber_temp = chamber_temp > 0 and not "{print_chamber_temperature}" in Profile.regex_strip_comments.sub("", contents)

			if include_chamber_temp:
				prefix += "M141 S{chamber_temp}\n".format(chamber_temp=chamber_temp)
			# zyd for add chamber -e
