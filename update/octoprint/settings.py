...
default_settings = {
	...
	"temperature": {
		"profiles": [
			{"name": "ABS", "extruder" : 210, "bed" : 100, "chamber" : 80 },
			{"name": "PLA", "extruder" : 180, "bed" : 60, "chamber" : 50 }
		],
		"cutoff": 30
	},
	"printerProfiles": {
		...
		"defaultProfile": {
			...
			"heatedBed":True,
			"heatedChamber":True,#zyd for add chamber
			"axes": {
				"x": {"speed": 600, "inverted": False},
				"y": {"speed": 600, "inverted": True},
				"z": {"speed": 600, "inverted": True},
				"e": {"speed": 300, "inverted": False}
			},
			"extruder": {
				"count": 2, # add by evan, for change tool-numbers
				"offsets": [
					[0,0],
				],
				"nozzleDiameter": 0.4
	...
	"api": {
		"enabled": True,
		"key": "6A6C8EF5D1E54C878A2848166277C4C0",
	...
	"plugins": {
		"_disabled": ["announcements","softwareupdate"]
	},
	"scripts": {
		"gcode": {
...
class Settings(object):
	...
	def _migrate_config(self):
		...
		migrators = (
			...
			# zyd remove for not migrate printerParameters -s
			#self._migrate_printer_parameters,
			# zyd remove for not migrate printerParameters -e
		...
		if "scripts" in self._config:
			if "gcode" in self._config["scripts"]:
				...
				replacements = dict(
					...
					disable_bed="M140 S0",
					disable_chamber="M141 S0", # zyd for add chamber
					disable_fan="M106 S0"
				)
	...
	def getUsbFolder(self, dirname):
		if sys.platform == "win32":
			folder = os.path.join(os.environ["APPDATA"], dirname.lower())
		else:
			folder = os.path.expanduser(os.path.join("~", dirname.lower()))
