...
/*
  ...
  Com::println();
  // check the response
  Com::printFLN("PEER_INITIATOR");
  for(uint8_t i=0;i<19+6;i++)
  {
    Com::printFLN(pn532_packetbuffer[i], HEX); 
    Com::print(" ");
  }
#endif
*/
...
boolean PN532::initiatorTxRx(char* dataOut,char* dataIn)
{
  ...
  Com::println();
  // check the response
  Com::printFLN("INITIATOR receive:");
...
uint32_t PN532::targetTxRx(char* dataOut,char* dataIn)
{
  ...
  if(pn532_packetbuffer[7] == 0x00) //If no errors in receiving, send data.
  {
    ...
    Com::println();
    // check the response
    Com::printFLN("TARGET get response after transmiting: ");
...
uint32_t PN532::authenticateBlock(uint8_t cardnumber /*1 or 2*/,uint32_t cid /*Card NUID*/, uint8_t blockaddress /*0 to 63*/,uint8_t authtype/*Either KEY_A or KEY_B */, uint8_t * keys) 
{
  ...
  {
    Com::printFLN(pn532_packetbuffer[iter], HEX);
    Com::print(" ");
  }
  Com::println();
  // check some basic stuff
...
uint32_t PN532::readPassiveTargetID(uint8_t cardbaudrate) 
{
  ...
  //Com::printFLN(Com::tFound); 
 // Com::printFLN(pn532_packetbuffer[7]); 
 // Com::printFLN(Com::ttags);
  ...
  //Com::printFLN(Com::tSens_Response);  
  //Com::printFLN(sens_res, HEX);

  //Com::printFLN(PSTR("Sel Response: 0x"));
  //Com::printFLN(pn532_packetbuffer[11], HEX);
 
  cid = 0;
  for (uint8_t i=0; i< pn532_packetbuffer[12]; i++) {
    cid <<= 8;
    cid |= pn532_packetbuffer[13+i];
   // Com::printFLN(Com::t0x); 
	//Com::printFLN(pn532_packetbuffer[13+i], HEX);
    
  }

#ifdef PN532DEBUG
  Com::printFLN("TargetID");
...
#ifdef PN532DEBUG
  Com::printFLN(Com::t0x); 
  Com::printFLN(PN532_PREAMBLE, HEX);
  Com::printFLN(Com::t0x); 
  Com::printFLN(PN532_PREAMBLE, HEX);
  Com::printFLN(Com::t0x); 
  Com::printFLN(PN532_STARTCODE2, HEX);
  Com::printFLN(Com::t0x); 
  Com::printFLN(cmd_len, HEX);
  Com::printFLN(Com::t0x); 
  Com::printFLN(cmdlen_1, HEX);
  Com::printFLN(Com::t0x); 
  Com::printFLN(PN532_HOSTTOPN532, HEX);
#endif
...
#ifdef PN532DEBUG
  Com::printFLN(Com::t0x); 
  Com::printFLN(checksum_1, HEX);
  Com::printFLN(Com::t0x); 
  Com::printFLN(PN532_POSTAMBLE, HEX);
  Com::println();
#endif
} 
