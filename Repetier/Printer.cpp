...
#include "pins.h"
#include "PN532.h"
#include "Commands.h"//add by andy
...
#error You have defined hardware z max endstop without pin assignment. Set pin number for Z_MAX_PIN
    ...
    SET_OUTPUT(FAN_PIN);
    WRITE(FAN_PIN, LOW);
	//setFanSpeedDirectly(255);//add by Andy
#endif
#if FAN2_PIN > -1 && FEATURE_FAN2_CONTROL
	SET_OUTPUT(FAN2_PIN);
	WRITE(FAN2_PIN, LOW);
	//setFan2SpeedDirectly(255);//add by Andy
#endif
#if FAN_THERMO_PIN > -1
...
//add by andy
#if defined(PUMP_ENABLE_PIN)&&PUMP_ENABLE_PIN>-1
    SET_OUTPUT(PUMP_ENABLE_PIN);
    WRITE(PUMP_ENABLE_PIN, LOW);
#endif
#if defined(Temperature_Alarm_State_PIN)&&Temperature_Alarm_State_PIN>-1
    SET_OUTPUT(Temperature_Alarm_State_PIN);
    WRITE(Temperature_Alarm_State_PIN, LOW);
#endif
//end
