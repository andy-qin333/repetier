...
void Commands::processMCode(GCode *com) {
	PN532 nfc(PN532_CS);
	AT24CXXX ROM_512;
	HX711 hx(18, 19, 128,0.0021147729579752);
	uint8_t EPR_UVLED_PWM;
	switch( com->M ) {
        ...
        case 106: // M106 Fan On
            if(!(Printer::flag2 & PRINTER_FLAG2_IGNORE_M106_COMMAND)) {
                if(com->hasP() && com->P == 1)
					...
					/*Led control signal inversionand, by Andy*/
					//if(com->hasS()&& com->S==255)
					//	setFanSpeed(50);
					 if(com->hasS()&& com->S==0)
						setFanSpeed(0);
					else
                    setFanSpeed(HAL::eprGetByte(EPR_UVLED_PWM_POS));
#ifdef NANODLP
        ...
        case 107: // M107 Fan Off
            if(!(Printer::flag2 & PRINTER_FLAG2_IGNORE_M106_COMMAND)) {
				...
				//setFan2Speed(255);
				    ...
				    setFanSpeed(0);//and by Andy
			}
            break;
#endif
				 
		           case 266: // M266 OPEN Water pump
		   if(com->hasS() && com->S == 0)
                   setFan2Speed(0);
	       else
		   //	setFan2Speed(com->hasS() ? com->S : 255);
            setFan2Speed(255);
		 	...
		 	break;
			       case 267: // M267 Save the PWM value
			       
			  EPR_UVLED_PWM=(com->hasS() ? com->S : 255);
				 
				HAL::eprSetByte(EPR_UVLED_PWM_POS,EPR_UVLED_PWM);	
				Com::printFLN(PSTR("EPR_UVLED_PWM_VAL_Write_Successful!"));
				Com::printFLN(Com::tUVLED_PWM,HAL::eprGetByte(EPR_UVLED_PWM_POS));
				break;

			 	   case 268: // M267 Read the PWM value
			       
				  EPR_UVLED_PWM=(HAL::eprGetByte(EPR_UVLED_PWM_POS));
				  // Com::printFLN(PSTR("EPR_UVLED_PWM_VAL:"));
				  Com::printFLN(Com::tUVLED_PWM,EPR_UVLED_PWM);
			    break;

				   case 269://M269 BOARD_FAN Control  
					   if(com->hasS() && com->S == 0)
							pwm_pos[PWM_BOARD_FAN]=0;
					   else  
					   	pwm_pos[PWM_BOARD_FAN]=(com->hasS() ? com->S : 255);
				   
					   break;

			       case 270: // M270 weighing
         
		{
           // hx.set_offset (69480);
			// hx.set_offset (123213.4);
		   hx.set_offset(HAL::eprGetDouble(EPR_WEIGHT_OFFSET));
		
		 	 //if(digitalRead(34) == HIGH) hx.tare();
               double sum0 = 0;
			   double sum1 = 0;
               for (int i = 0; i < 10; i++) {
                  sum0 += hx.bias_read();
			//sum0 += hx.read();//peel read
                 }
			   sum1=(sum0/10);
			   Com::printF(Com::tWeight,sum1,2);
			   Com::printFLN(Com::tg);
			 //  Com::printFLN(Com::tare());
			}
            break;
	 ...
	 if (com->hasS()){
        if(com->S == 0){
			 Com::printFloat(HAL::eprGetFloat(EPR_WEIGHT_OFFSET),3);
		     Com::print(' ');
        	}
		else
			{
		 	...
		 	v=hx.tare();   
            hx.set_offset(v); 
		 	HAL::eprSetDouble(EPR_WEIGHT_OFFSET, v);
		 	}
			 	}
      break;
	  
	case 272://NFC Memory  Wirte
/*	 nfc.begin();
         uint32_t versiondata;
         versiondata = nfc.getFirmwareVersion();
        // Com::printFLN(Com::tg,versiondata);  
     if (!versiondata) {
#ifdef NFC_DEMO_DEBUG
      Com::printFLN(PSTR("Didn't find PN53x board"));
#endif
     while (1); // halt
  	}
#ifdef NFC_DEMO_DEBUG
	Com::printFLN(PSTR("Find chip PN532"));	
#endif
         nfc.SAMConfig();
     */    
         uint32_t id;
  ...
  {
#ifdef NFC_DEMO_DEBUG
     //   Com::printFLN(PSTR("Read card #")); 
   // Com::printFLN(Com::tRead_card,id);
#endif  
      uint8_t keys[]= { 0xFF,0xFF,0xFF,0xFF,0xFF,0xFF };
  uint8_t written;
  written=0;
	uint8_t writeBuffer[16];
	   writeBuffer[0]=(com->hasS() ? com->S : 200);
	   for(uint8_t i =1;i < 16;i ++)
	   { 
		 writeBuffer[i]=0; //Fill buffer with 0,1,2....F
	   }
		 if(nfc.authenticateBlock(1, id ,0x08,KEY_A,keys)) //authenticate block 0x08
		   {
             ...
             Com::printFLN(PSTR("Write_Successful"));
			...
			break;
	  case 273://NFC Memory read	  	
 /*      nfc.begin();
         uint32_t versiondata1;
         versiondata1 = nfc.getFirmwareVersion();
        // Com::printFLN(Com::tg,versiondata1);  
     if (!versiondata1) {
#ifdef NFC_DEMO_DEBUG
      Com::printFLN(PSTR("Didn't find PN53x board"));
#endif
	...
	*/	 
         uint32_t id1;
  // look for MiFare type cards
       id1 = nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A);

         if (id1 != 0) 
  {
#ifdef NFC_DEMO_DEBUG
     //   Com::printFLN(PSTR("Read card #")); 
  //  Com::printFLN(Com::tRead_card,id1);
#endif  
      uint8_t keys[]= { 0xFF,0xFF,0xFF,0xFF,0xFF,0xFF };
    if(nfc.authenticateBlock(1, id1 ,0x08,KEY_A,keys)) //authenticate block 0x08
    {
      //if authentication successful
      uint8_t block[16];
...
#ifdef NFC_DEMO_DEBUG
	Com::printF(PSTR("Bottle_Resin_Type:"));
 ...
 delay(500);
	   break; 
	  case 274://Trough_Resin_Type_Read
	      uint8_t  Type;
		  Wire.begin(); 	   
		  ROM_512.Memory_Size(2);
		  Type = ROM_512.ReadByte(0x50 ,0x0000);
	 //Com::printFLN(PSTR("Trough_Resin_Type:"));
	 Com::printFLN(Com::tTrough_Resin_Type,Type);
	   break; 

	   case 275://Trough_Resin_Type_Wirte
	   
      uint8_t  Type1;
	   Type1=(com->hasS() ? com->S : 200);;
	   Wire.begin(); 	  
		  ROM_512.Memory_Size(2);
	   ROM_512.WriteByte(0x50 ,0x0000 ,Type1);
 Com::printFLN(PSTR("Write_Successful"));

	   break; 
      case 276://State level reading
       Endstops::update();
            Endstops::update();
            Com::printF(Com::tLiquidLevel);
            Com::printF(Endstops::zProbe() ? Com::tHSpace : Com::tLSpace);
            Com::println();
            break;

	 case 277://pump open/close
      #if ((PUMP_ENABLE_PIN)>-1)
	            if(com->S == 1)
          WRITE(PUMP_ENABLE_PIN, HIGH);
	   else
	   	  WRITE(PUMP_ENABLE_PIN, LOW);
      
       #endif
		 	break;
	   	 case 278://Door limit read   
	   	    Endstops::update();
			Endstops::update();
            Com::printF(Com::tDoor_Limit);
            Com::printF(Endstops::xMin() ? Com::tHSpace : Com::tLSpace);
            Com::println();
   
		 	break;
		 case 279://  read  Temperature_Alarm_State
    
            Com::printF(Com::tTemperatureAlarmState);
            Com::printF(READ(Temperature_Alarm_State_PIN) ? Com::tHSpace : Com::tLSpace);
            Com::println();
