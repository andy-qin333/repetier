    ...
    for(uint8_t controller = 0; controller < NUM_TEMPERATURE_LOOPS; controller++)
    {
        ...
        { 
            ...
            else
            {
				...
				Com::printErrorFLN(PSTR("The led board overheating, please add water cooling fluid!")); //and by andy
				WRITE(Temperature_Alarm_State_PIN,HIGH);
	            if(!Printer::isAnyTempsensorDefect())
	            {
					newDefectFound = true;
		            Printer::setAnyTempsensorDefect();
...
#define NUMTEMPS_15 27 // DYZE DESIGN 500掳C Thermistor
        ...
        if(heating == false && currentTemp < temp)
        {
            if(time - t1 > (controllerId < NUM_EXTRUDER ? 5000 : 3000))
            {
                ...
                if(cycles > 0)
                {
                    ...
                    if(cycles > 2)
                    {
                        // Parameter according Ziegler隆搂CNichols method: http://en.wikipedia.org/wiki/Ziegler%E2%80%93Nichols_method
